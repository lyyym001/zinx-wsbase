...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	global.Glog.Info("Init timerWhell name = " + tw.name + " is Done!")
	return tw
}
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	global.Glog.Info("Add timerWhell[" + tw.name + "]'s next [" + next.name + "] is succ!")
}
...
func (tw *TimeWheel) Run() {
	go tw.run()
	global.Glog.Info("timerwheel name = " + tw.name + " is running...")
}
