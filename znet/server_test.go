...
func ClientTest(i uint16) {
	...
	for {
		dp := NewDataPack()
		msg, _ := dp.Pack(NewBinaryMsgPackage(i, []byte("client test message")))
...
func (router *PingRouter) Handle(request ziface.IRequest) {
	...
	err := request.GetConnection().SendBinaryMsg(1, []byte("ping...ping...ping\n"))
	if err != nil {
...
func (router *HelloRouter) Handle(request ziface.IRequest) {
	...
	err := request.GetConnection().SendBinaryMsg(2, []byte("hello zix hello Router"))
	if err != nil {
		fmt.Println(err)
	}
}
...
func DoConnectionBegin(conn ziface.IConnection) {
	...
	err := conn.SendBinaryMsg(2, []byte("DoConnection BEGIN..."))
	if err != nil {
		fmt.Println(err)
	}
}
